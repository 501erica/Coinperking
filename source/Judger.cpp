...
CalculateMode Judger::judge(date date)
{
    ...
    {
        if ((date.information.tm_mon + 1 == specialdaylist.at(i).first) &&
            (date.information.tm_mday == specialdaylist.at(i).second))
        {
            return CalculateMode::SPECIAL_DAY;
        }
    }
...
bool Judger::judge(std::string InDate, std::string OutDate)
{
    ...
    {
        std::cout << "入力形式が不正です。以下の形式で日時を半角で入力してください"
                  << "yyyy/mm/dd-hh:mm" << std::endl;
        return false;
    }

    auto rx2 = std::regex{
        R"(([1-2][0-9][0-9][0-9])(\.|-|/)(0[1-9]|1[0-2])(\.|-|/)(0[1-9]|[12][0-9]|3[01])(\.|-|/| )([01][0-9]|2[0-3])(:|-|/)([0-5][0-9]))"};
    if (!(std::regex_match(InDate.c_str(), rx2) && std::regex_match(OutDate.c_str(), rx2)))
    {
        std::cout << "入力形式が不正です。入力年は1000~2999,入力月は01~12,入力日は01~"
                     "31で入力してください。"
                  << std::endl;
        return false;
    }
    ...
    if (Intime > Outtime)
    {
        std::cout << "日時指定が不正です。出庫日時に入庫時刻より過去の時間が指定されています。"
                  << std::endl;
        return false;
    }
